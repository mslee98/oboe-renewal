...
// floorPlan 데이터를 sceneData 형식으로 변환하는 함수
const floorPlanToSceneData = (corners, walls, rooms) => {
  const sceneData = [
    {
      id: 'floorplan-root',
      name: 'FloorPlan',
      ...
      children: [
        {
          ...
          children: corners.map(corner => ({
            id: corner.archiId,
            name: `Corner ${corner.archiId.slice(0, 8)}`,
            type: 'corner',
            visible: true,
            position: {
              x: corner.position.x,
        ...
        },
        {
          ...
          children: walls.map(wall => ({
            id: wall.archiId,
            name: `Wall ${wall.archiId.slice(0, 8)}`,
            type: 'wall',
            visible: true
          }))
        },
        {
          ...
          children: rooms.map(room => ({
            id: room.archiId,
            name: `Room ${room.archiId.slice(0, 8)}`,
            type: 'room',
            visible: true
          }))
        }
...
// floorPlan 데이터를 SceneContext에 주입하는 컴포넌트 (더 이상 사용하지 않음 - Interior3D에서 직접 처리)
// const FloorPlanSceneInjector = ({ corners, walls, rooms }) => {
//   const { updateSceneData } = useScene();
//   
//   useEffect(() => {
//     // floorPlan 데이터를 sceneData 형식으로 변환
//     const sceneData = floorPlanToSceneData(corners, walls, rooms);
//     
//     // SceneContext에 주입 (originalScene은 null로 설정 - floorPlan은 3D 객체가 아니므로)
//     updateSceneData(sceneData, null);
//   }, [corners, walls, rooms, updateSceneData]);
//   
//   return null;
// };
...
const Interior2dsContent = () => {
  ...
  const handleToggleEditorSidebar = () => setIsEditorSidebarOpen(v => !v);
    ...
    // 3D 에디터(패널 포함)로 이동
    const handleOpen3DEditorPanel = () => {
      const snapshot = {
        corners: corners.map(c => ({ archiId: c.archiId, position: { ...c.position } })),
        walls: walls.map(w => ({ ...w })),
        rooms: rooms.map(r => ({ ...r }))
      };
      try {
        localStorage.setItem('floorplanSnapshot', JSON.stringify(snapshot));
      } catch (_) {}
      navigate('/interior3d-editor');
    };
